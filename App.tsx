...
--- TEXTO REFINADO ---`;
  ...
  return (
    ...
    <Modal isOpen={isNewDocModalOpen} onClose={() => setIsNewDocModalOpen(false)} title="Criar Novo Documento" maxWidth="max-w-3xl">
      <div className="space-y-4">
        ...
        <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                ...
                {etpTemplates.map(template => (
                    <button 
                        key={template.id}
                        onClick={() => handleCreateFromTemplate(template)}
                        className="w-full text-left p-4 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors border border-blue-200 flex flex-col justify-between h-full"
                    >
                        <div>
                            <div className="flex items-center gap-3">
                                <Icon name="file-alt" className="text-blue-500 text-xl" />
                                <p className="font-bold text-blue-800">{template.name}</p>
                            </div>
                            <p className="text-sm text-blue-700 mt-2 pl-8">{template.description}</p>
                        </div>
                    </button>
                ))}
            </div>
        ...
        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                ...
                {trTemplates.map(template => (
                    <button 
                        key={template.id}
                        onClick={() => handleCreateFromTemplate(template)}
                        className="w-full text-left p-4 bg-purple-50 hover:bg-purple-100 rounded-lg transition-colors border border-purple-200 flex flex-col justify-between h-full"
                    >
                        <div>
                             <div className="flex items-center gap-3">
                                <Icon name="gavel" className="text-purple-500 text-xl" />
                                <p className="font-bold text-purple-800">{template.name}</p>
                            </div>
                            <p className="text-sm text-purple-700 mt-2 pl-8">{template.description}</p>
                        </div>
                    </button>
                ))}
            </div>
